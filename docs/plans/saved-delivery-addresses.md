...
> Статус: **реалізовано + протестовано наживо** (2026-07-08). Backend + frontend зібрані, типи
> чисті, lint без нових помилок, міграцію застосовано до dev-БД. Живий тест повного циклу пройдено:
> 20/20 API-перевірок (curl) + 36/36 UI-перевірок (Playwright, headless Chromium) — див.
> «Живе тестування» нижче. Коміти фічі досі **не запушені** (рішення користувача).
...
## Живе тестування (2026-07-08, пройдено повністю)

- **API (curl, 20/20):** порожній список → перша адреса авто-`isDefault` → друга не default,
  основна перша у списку → `PATCH :id/default` тримає інваріант «одна основна» → часткове
  оновлення не губить НП-референси → видалення основної підвищує найновішу → чужа адреса 404,
  без кукі 401.
- **UI (Playwright headless, 36/36), усі 4 сценарії з плану:**
  1. Перший checkout: чекбокс «Зберегти в профіль» авто-увімкнений (адрес нема), після оформлення
     адреса зʼявилась (cityRef/warehouseRef/recipient/phone, телефон нормалізовано у `+380…`).
  2. Повторний checkout: блок «Збережені адреси» зверху, основна автовибрана, підставляє
     ПІБ + телефон + доставку, «Доставка» — резюме + «Змінити»; адреса не дублюється.
  3. Профіль: додавання/редагування/«зробити основною»/видалення основної з підвищенням іншої.
  4. Редагування НП-адреси: підказка «Поточна: …», збереження без перевибору не губить refs,
     перевибір відділення оновлює `warehouseRef`.
- **Снапшот незалежний (ADR-0013) — підтверджено:** замовлення `TL-2026-000016` зберегло
  відділення на момент оформлення, подальше редагування адреси на нього не вплинуло.
- **Тестові дані у dev-БД:** юзери `test-addresses-20260708@example.test`,
  `test-addresses-b-20260708@example.test` (адреси зачищені); замовлення
  `TL-2026-000013…16` з коментарем «ТЕСТ Claude … — можна видалити».
